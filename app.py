...
def build_system_prompt(style: str, gen_cover: bool, gen_interview: bool) -> str:
    style_notes = {
        ...
        "Academic": "Use precise language, emphasise publications, research, and methodologies if present.",
    }
    extras = []
    if gen_cover:
        extras.append(
...
def score_color(score: int):
    if score >= 75:
        return "#10b981", "#d1fae5"   # teal
    elif score >= 50:
        return "#f59e0b", "#fef3c7"   # amber
    else:
        return "#f97316", "#ffedd5"   # soft orange (never red)
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "AI Model",
        ["gpt-3.5-turbo", "gpt-4"],
        help="GPT-4 gives richer results but is slower.",
    )

    st.divider()

    if st.button("🗑️ Start Fresh", use_container_width=True):
        confirm = st.warning("This will clear all inputs and results. Click again to confirm.")
...
if key_error == "missing_key":
    st.error(
        ...
        "1. Open **Settings → Secrets** in Streamlit Cloud (or create `.streamlit/secrets.toml` locally).\n"
...
if analyze_clicked:
    ...
    if missing:
        ...
        if raw == "RATE_LIMIT":
            ...
            st.error("Authentication failed. Please check that your `OPENAI_API_KEY` secret is valid.")
                ...
                if learning_path:
                    ...
                    for item in learning_path:
                        if isinstance(item, dict):
                            ...
                            st.markdown(f"**{skill}**")
                            for r in resources:
                                # Try to render as a clickable link if URL present
